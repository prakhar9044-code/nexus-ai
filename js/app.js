...
const DocHelper = (() => {
    ...
    function printPDF() {
        ...
        if (!msgs || !msgs.children.length) { Toast.show('No messages to export', 'info'); return; }
        const win = window.open('', '_blank');
        const title = document.querySelector('.chat-history .active')?.textContent || 'Nexus Chat';
        win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${title} — Nexus Export</title>
        <style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Segoe UI',Arial,sans-serif;max-width:800px;margin:40px auto;padding:0 24px;color:#1a1a2e;line-height:1.7}
        .header{text-align:center;padding-bottom:20px;border-bottom:2px solid #4a6cf7;margin-bottom:24px}
        .header h1{font-size:1.4rem;color:#4a6cf7}.header p{font-size:0.8rem;color:#888;margin-top:4px}
        .msg{margin-bottom:16px;padding:12px 16px;border-radius:12px}.msg-user{background:#eef2ff;border-left:3px solid #4a6cf7}
        .msg-ai{background:#f8f9fb;border-left:3px solid #22c55e}.msg-role{font-size:0.72rem;font-weight:700;margin-bottom:4px;text-transform:uppercase;letter-spacing:0.5px}
        .msg-user .msg-role{color:#4a6cf7}.msg-ai .msg-role{color:#22c55e}
        pre{background:#1e1e2e;color:#e0e0e0;padding:12px;border-radius:8px;overflow-x:auto;margin:8px 0;font-size:0.85rem}
        code{background:#f0f0f0;padding:1px 5px;border-radius:3px;font-size:0.88rem}
        .footer{margin-top:40px;padding-top:16px;border-top:1px solid #e5e7eb;text-align:center;font-size:0.75rem;color:#999}
        @media print{body{margin:20px}}
        </style></head><body>
        <div class="header"><h1>📄 ${title}</h1><p>Exported from Nexus • ${new Date().toLocaleDateString()} • Developed by Prakhar</p></div>`);
        msgs.querySelectorAll('.message').forEach(m => {
            ...
            if (!bubble) return;
            win.document.write(`<div class="msg ${isUser ? 'msg-user' : 'msg-ai'}"><div class="msg-role">${isUser ? '👤 You' : '🤖 Nexus'}</div>${bubble.innerHTML}</div>`);
        });
        win.document.write('<div class="footer">Generated by Nexus — nexuspv.netlify.app</div></body></html>');
        win.document.close();
        setTimeout(() => { win.print(); }, 500);
        Toast.show('📄 Print/PDF dialog opened!', 'success');
    }

    return { downloadText, downloadHTML, printPDF };
})();
...
const App = (() => {
            ...
            try {
                ...
                setTimeout(() => {
                    try {
                        ...
                        // Phase 14 modules
                        if (typeof Threads !== 'undefined') Threads.init();
    ...
    function setupNav() {
        ...
        // Phase 9: Search, Theme, PDF buttons
        ...
        // Phase 14: Planner, Insights, Threads buttons
